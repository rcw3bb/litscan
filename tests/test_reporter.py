...
def test_write_json_creates_file(tmp_path: Path) -> None:
    ...
    _write_json(groups, out, "2026-01-01 00:00:00", ["src"], 0, "both", [])
    assert out.exists()
...
def test_write_json_structure(tmp_path: Path) -> None:
    ...
    out = tmp_path / "report.json"
    _write_json(groups, out, "2026-05-31 12:00:00", ["src"], 0, "both", [])
...
def test_write_json_metadata_content(tmp_path: Path) -> None:
    """The JSON file should contain paths-scanned, min-count, mode, and literals."""
    groups = [_make_group("42", 1, ["main.py:10:4"])]
    out = tmp_path / "report.json"
    _write_json(
        groups, out, "2026-05-31 12:00:00", ["src", "tests"], 2, "number", ["42"]
    )
    data = json.loads(out.read_text(encoding="utf-8"))
    assert data["paths-scanned"] == ["src", "tests"]
    assert data["min-count"] == 2
    assert data["mode"] == "number"
    assert data["literals"] == ["42"]
...
def test_build_html_returns_string(tmp_path: Path) -> None:
    ...
    result = _build_html(groups, "2026-01-01 00:00:00", ["src"], 0, "both", [])
...
def test_build_html_contains_metadata(tmp_path: Path) -> None:
    """The HTML output should include paths scanned, min count, mode, and literals."""
    groups = [_make_group("'a'", 1, ["f.py:1:0"])]
    result = _build_html(
        groups, "2026-01-01 00:00:00", ["src", "tests"], 3, "string", ["a", "b"]
    )
    assert "src" in result and "tests" in result
    assert "Minimum count: 3" in result
    assert "Mode: string" in result
...
def test_write_html_creates_file(tmp_path: Path) -> None:
    ...
    _write_html(groups, out, "2026-01-01 00:00:00", ["src"], 0, "both", [])
    assert out.exists()
...
def test_write_outputs_json_format(tmp_path: Path) -> None:
    ...
    written = write_outputs(groups, tmp_path, "out", "json", paths_scanned=["src"])
...
def test_write_outputs_html_format(tmp_path: Path) -> None:
    ...
    groups = [_make_group("1", 1, ["f.py:1:0"])]
    written = write_outputs(groups, tmp_path, "out", "html", paths_scanned=["src"])
    assert len(written) == 1
...
def test_write_outputs_all_format(tmp_path: Path) -> None:
    ...
    groups = [_make_group("1", 1, ["f.py:1:0"])]
    written = write_outputs(groups, tmp_path, "out", "all", paths_scanned=["src"])
...
def test_write_outputs_creates_missing_directory(tmp_path: Path) -> None:
    ...
    write_outputs(groups, out_dir, "report", "json", paths_scanned=["src"])
...
def test_write_outputs_returns_paths(tmp_path: Path) -> None:
    ...
    groups = [_make_group("'x'", 1, ["f.py:1:0"])]
    written = write_outputs(groups, tmp_path, "result", "all", paths_scanned=["src"])
    assert all(isinstance(p, Path) for p in written)


def test_write_outputs_defaults_metadata(tmp_path: Path) -> None:
    """write_outputs should default min-count to 0, mode to 'both', literals to []."""
    groups = [_make_group("'x'", 1, ["f.py:1:0"])]
    write_outputs(groups, tmp_path, "out", "json", paths_scanned=["src"])
    data = json.loads((tmp_path / "out.json").read_text(encoding="utf-8"))
    assert data["min-count"] == 0
